...
found:
  ...
  if((p->kstack = kalloc()) == 0){ //kalloc ; kernel allocation 
    p->state = UNUSED;
    return 0;
  }
...
{
  int i, pid;
  struct proc *np; // new process -- mohamed 
  struct proc *curproc = myproc(); // equal to this to point to himself 
...
// function to sum the the tickets of process --> mohamed ali
int
lottery_total(void){
  struct proc *p;
  int ticket_aggregate=0; // counter
...
{
  struct proc *p;
  //Eable interrupts on this processor.
  sti();
